  ...
  $(document).ready(function () {
    ...
    if ($(".mobile-menu-items").length) {
      $(".mobile-menu-items").meanmenu({
        // Both #sidebar-area and .mobile-side-menu contain a ".side-menu-wrap"
        // div. meanmenu's container option is a raw jQuery selector, so an
        // unscoped ".side-menu-wrap" matched BOTH and injected/duplicated the
        // mean-bar + nav markup into both panels' internal DOM/width state at
        // once — that cross-contamination is what broke the mobile menu's
        // sizing/positioning. Only #sidebar-area is actually reachable on
        // this site (opened via .sidebar-trigger in the header); no page
        // ships a .mobile-side-menu-toggle button, so .mobile-side-menu is
        // unreachable dead markup. Scope to the drawer users can open.
        meanMenuContainer: "#sidebar-area .side-menu-wrap",
        meanScreenWidth: "992",
